...
if(isset($_POST['subject'],$_POST['type'],$_POST['text'])){
	...
	if(empty($subject) || empty($type) || empty($text) || mb_strlen($subject) > 212 || mb_strlen($text) > 2000 || mb_strlen($type) > 1 || !is_numeric($type) || $type < 1 || $type > 4){
		...
		$stmt = $conn->query("SELECT id FROM ticket WHERE creator={$_SESSION['memberId:wse6']} AND status <> 0");
		if($stmt->rowCount() > 0){
			returnJSON(array('t' => 'خطأ','m' => 'لا يمكنك فتح اكثر من تذكره يجب اغلاق التذكره الأولى','tp' => 'error','b' => true));		
		}else{
			if($type == 3){
				if(!isset($_POST['wse6'])){
					returnJSON(array('t' => 'خطأ','m' => 'حدث خطأ غير معروف','tp' => 'error','b' => false));		
				}elseif(strlen($_POST['wse6']) > 32 || strlen($_POST['wse6']) < 2 || empty($_POST['wse6'])){
					returnJSON(array('t' => 'خطأ','m' => 'إسم الوسيط خاطئ','tp' => 'error','b' => true));		
				}else if(!preg_match("/^[A-Za-z0-9_-]+$/", $_POST['wse6'])){
					returnJSON(array('t' => 'خطأ','m' => 'يجب إن يكون اسم الوسيط مكون من حروف إنجليزية وارقام و - و _ فقط','tp' => 'error','b' => true));		
				}else{
					$sql = $conn->prepare("SELECT id,rank FROM Customers WHERE username=:username AND isStaff=1 AND rank >= 1 AND rank <= 2");
					$sql->bindValue(':username', $_POST['wse6']);
					$sql->execute();
					if($sql->rowCount() == 0){
						returnJSON(array('t' => 'خطأ','m' => 'لم يتم العثور على هذا الوسيط','tp' => 'error','b' => true));		
					}else{
						$wse6data = $sql->fetch();
						$time = time();
						$insertData = $conn->prepare("INSERT INTO ticket (creator, title, msg, type, status, time,forwho,for_rank) VALUES (:creator,:title,:msg,{$type}, 1, {$time}, ".$wse6data['id'].", ".$wse6data['rank'].")");
...
if(isset($_POST['read'])){
	...
	if(!ctype_digit($_POST['read']) || strlen($_POST['read']) > 20){
		...
		$conn=$database->openConnection();
		$check = $conn->prepare('SELECT t.id,t.creator,t.title,t.msg,t.type,t.status,t.time,tr.reply ticketReply,tr.reply_time reply_time, tr.replyfrom, tr.from_rank FROM ticket t LEFT JOIN ticket_replies tr ON t.id = :id AND tr.tid = :id WHERE t.creator = '.$_SESSION['memberId:wse6'].' AND t.id = :id ORDER BY tr.id DESC');
		$check->bindValue(":id", $_POST['read'], PDO::PARAM_INT);
		$check->execute();
		if($check->rowCount() > 0){
			$dataC = $conn->query('SELECT username,img FROM Customers WHERE id = '.$_SESSION['memberId:wse6'].'')->fetch();
                ...
                <tbody>
                    ...
                    <tr>
						...
						<td class="d-none d-sm-table-cell text-center" style="width: 140px;">
                            ...
                            <small>'.htmlspecialchars($dataC['username']).'<br>'.getRank($_SESSION['memberId:wse6']).'</small>
                        </td>
                    </tr>
...
if(isset($_POST['checkreply'])) {
	...
	$conn = $database->openConnection();
	$check = $conn->query('SELECT id,status FROM ticket WHERE creator = '.$_SESSION['memberId:wse6']);
	if($check->rowCount() > 0){
		$data = $check->fetch();
		if($data['status'] == 2){
			$update = $conn->query('UPDATE ticket SET status = 3 WHERE id = '.$data['id'].'');
			$insert = $conn->prepare('INSERT INTO ticket_replies (tid, reply,replyfrom,reply_time,from_rank) VALUES ('.$data['id'].', :reply, '.$_SESSION['memberId:wse6'].', '.time().', 0)');
...
if(isset($_POST['updatestatus'])){
	...
	$conn = $database->openConnection();
	$countStatus= $conn->query('Select (select count(*) from ticket WHERE creator = '.$_SESSION['memberId:wse6'].' AND status = 1 ) as OpenedCount, (select count(*) from ticket WHERE creator = '.$_SESSION['memberId:wse6'].' AND status = 2) as replySupport, (select count(*) FROM ticket WHERE creator = '.$_SESSION['memberId:wse6'].' AND status = 3) as customerReply, (SELECT count(*) FROM ticket WHERE creator = '.$_SESSION['memberId:wse6'].' AND status = 0) as closedCount');
...
if(isset($_POST['closeTicket'])){
	...
	if(!ctype_digit($_POST['closeTicket']) OR mb_strlen($_POST['closeTicket']) > 50){
		...
		$conn = $database->openConnection();
		$check = $conn->query('SELECT status FROM ticket WHERE id = '.$_POST['closeTicket'].' AND creator = '.$_SESSION['memberId:wse6']);
		if($check->rowCount() > 0){
			$data = $check->fetch();
			if($data['status'] != 0){
