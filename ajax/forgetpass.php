		...
		if(mb_strlen($_POST['email']) > 128) {
			...
			if($stmt->rowCount() > 0){
				...
				if($antiSpam < time()){
					...
					$sql->execute();
					if($sql->rowCount() > 0){
						$body="<!DOCTYPE html><html><head><meta charset='UTF-8'><title>WSE6 | Forget Pass</title></head><body dir='rtl'><p>مرحباً</p><p>لإعادة تعيين كلمة مرورك من فضلك إضغط على الزر.</p><p><a href='https://www.r2.ae/wse16/verify?t={$verficationToken}' target='_blank' rel='noopener noreferrer' style='display:block; text-decoration:none; text-align:center; border-radius:5px; padding:16px 20px; color:rgb(255,255,255); max-width:200px; line-height:16px; font-size:14px; background-color:rgb(0,176,116); margin:0 auto'>إعادة تعيين كلمة مروري</a></p></body></html>";
						$mail = new PHPMailer();
						$mail->CharSet = 'UTF-8';
						$mail->isSMTP();
