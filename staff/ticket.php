	...
	if($stmt->rowCount() > 0){
		...
		foreach($data as $da){
			...
			$chat .= '<tr class="table-active">
				...
				if($data[$last_key]['type'] == 3){ $text = '<button onclick="_upgradetoFounder('.$da['id'].')" type="button" class="btn-block-option pr-4" data-dismiss="modal"> رفع إلى المؤسس <i class="fa fa-arrow-left"></i></button>'; }
...
$conn = $database->openConnection();
$alltickets = $conn->query("SELECT * FROM ticket WHERE status != 0 AND type != 5 AND for_rank != 5 ORDER BY time DESC");
ob_end_flush();
 ?>
