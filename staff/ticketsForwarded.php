...
<div class="block-options" id="ticketbuttons"><a class="btn-block-option js-scroll-to-enabled  pl-4 pr-4" data-toggle="scroll-to" onclick="replyticket('.$da['id'].')">رد <i class="fa fa-reply"></i></a><button onclick="doThisTicketToAllow('.$da['id'].')" type="button" class="btn-block-option pl-4 pr-4" data-dismiss="modal" aria-label="Close"> السماح للوسيط بالرد <i class="fa fa-reply"></i> </button><button onclick="doThisTicket('.$da['id'].')" type="button" class="btn-block-option pl-4 pr-4" data-dismiss="modal" aria-label="Close"> اغلاق التذكرة <i class="si si-close"></i> </button><button onclick="_backTicketStaff()" type="button" class="btn-block-option pr-4" data-dismiss="modal"> الرجوع للخلف <i class="fa fa-arrow-left"></i></button></div>
</div>
<div class="block-content p-5" id="ticketInfo" data-toggle="slimscroll" data-height="350px">
		...
		exit(header('Location: ./ticketsForwarded'));
	}
} else {
$conn = $database->openConnection();
$alltickets = $conn->query("SELECT * FROM ticket WHERE type = 3 AND status != 0 AND for_rank = 5 AND forwho != 0");
ob_end_flush();
 ?>
...
<main id="main-container">
                ...
                <div class="content">
                    ...
                    <div class="block">
                        <div class="block-header block-header-default">
                            <h3 class="block-title">جميع التذاكر » <small>المحولة إلى المؤسس</small></h3>
                        </div>
                        <div class="block-content block-content-full">
                            <!-- DataTables init on table by adding .js-dataTable-full class, functionality initialized in js/pages/be_tables_datatables.js -->
                            <table class="table table-bordered table-striped table-vcenter js-dataTable-full" data-order='[[ 1, "asc" ]]' dir="ltr">
                                <thead>
                                    <tr>
...
function _backTicketStaff(){
	window.location = "ticketsForwarded";
}

function _upgradetoFounder(tid){
	sendData("ticket", "upgradetofounder="+tid,'POST')
...
function allowToReplyWset(tid){
	sendData("ticket", "allowtoreply="+tid,'POST')
		.then(function(response){
			toast({
			  type: response.tp,
			  title: response.t
...
function doThisTicketToAllow(tid){
	_sure('هل انت متاكد من انك تريد السماح للوسيط بإضافة رد ؟','info',allowToReplyWset,tid,'');
}
