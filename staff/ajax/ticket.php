...
if($_SERVER['REQUEST_METHOD'] == "POST"){
	...
	if(isset($_POST['closeTicket'],$_SESSION['staffId:wse6'])){
			...
			if($check->rowCount() > 0){
				...
				if($data['status'] != 0){
					if($data['for_rank'] == 5 OR $data['type'] == 5){
						$getRank = current($conn->query('SELECT rank FROM Customers WHERE id = '.$_SESSION['staffId:wse6'].'')->fetch());
						if($getRank != 5){
	...
	if(isset($_POST['allowtoreply'], $_SESSION['staffId:wse6'])) {
		...
		$conn = $database->openConnection();
		$stmt = $conn->query('SELECT rank FROM Customers WHERE id = '.$_SESSION['staffId:wse6'].'');
		if($stmt->rowCount() > 0){
	...
	if(isset($_POST['upgradetofounder'], $_SESSION['staffId:wse6'])){
		if(!ctype_digit($_POST['upgradetofounder'])){
			returnJSON(array('t'=>'حدث خطأ اثناء تنفيذ العملية.. حاول مججداً!','tp'=>'error'));
		} else if(antiSpam('ticketStaff:upgrade')){
			returnJSON(array('t' => 'خطأ','m' => 'من فضلك أنتظر قليلا بين محاولاتك','tp' => 'error','b' => 'موافق'));		
		}
		$conn = $database->openConnection();
		$stmt = $conn->query('SELECT forwho,for_rank,type FROM ticket WHERE id= '.$_POST['upgradetofounder'].' AND status != 0');
		if($stmt->rowCount() > 0){
			$data = $stmt->fetch();
			if($data['for_rank'] != 5 && $data['type'] == 3 && $data['forwho'] != 0){
				$update = $conn->query('UPDATE ticket SET for_rank=5 WHERE id= '.$_POST['upgradetofounder']);
				if($update->rowCount() > 0){
					returnJSON(array('t'=>'حسناً تم تحويل التذكرة إلى المؤسس','tp'=>'success'));
				} else {
					returnJSON(array('t'=>'حدث خطأ اثناء تحويل التذكرة..','tp'=>'error'));
				}
			} else {
				returnJSON(array('t'=>'التذكرة محولة بالفعل!','tp'=>'error'));
			}
		} else {
			returnJSON(array('t'=>'لم يتم العثور على التذكرة..','tp'=>'error'));	
		}
	...
	if(isset($_POST['checkreply'],$_POST['text'], $_SESSION['staffId:wse6'])) { /* 0 => Closed, 1 => Opened, 2 => Admin Reply , 3 => Customer Reply */
		...
		if($check->rowCount() > 0){
			...
			if($data['status'] == 0) {
				...
				if($data['for_rank'] == 5 && $data['needed_see'] != $_SESSION['staffId:wse6'] OR $data['type'] == 5){
					$getRank = current($conn->query('SELECT rank FROM Customers WHERE id = '.$_SESSION['staffId:wse6'].'')->fetch());
					if($getRank != 5){
				...
				$insert = $conn->prepare('INSERT INTO ticket_replies (tid, reply,replyfrom,reply_time,from_rank) VALUES ('.$data['id'].', :reply, '.$_SESSION['staffId:wse6'].', '.time().', 0)');
