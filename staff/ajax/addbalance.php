...
if($_SERVER['REQUEST_METHOD'] == "GET"){
	...
	if(isset($_GET['action'],$_GET['client'],$_GET['balance'],$_SESSION['staffId:wse6']) && $_GET['action'] == 'add'){
		if(antiSpam('addBalance:add')){
			returnJSON(array('t' => 'خطأ','m' => 'من فضلك أنتظر قليلا بين محاولاتك','tp' => 'error','b' => 'موافق'));		
		} else if(!rankPermission($_SESSION['staffId:wse6'],5,true)){
			returnJSON(array('t' => 'خطأ', 'm' => 'ليس لديك صلاحية لدخول هذه الصفحة', 'tp'=>'error'));
		}
		$conn=$database->openConnection();

		$isStaff=$conn->query('SELECT isStaff,rank FROM Customers WHERE id='.$_SESSION['staffId:wse6']);
