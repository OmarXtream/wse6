...
$sql = $conn->query("SELECT * FROM ticket WHERE creator={$_SESSION['memberId:wse6']} ORDER BY time DESC");
$countStatus= $conn->query('Select (select count(*) from ticket WHERE creator = '.$_SESSION['memberId:wse6'].' AND status = 1 ) as OpenedCount, (select count(*) from ticket WHERE creator = '.$_SESSION['memberId:wse6'].' AND status = 2) as replySupport, (select count(*) FROM ticket WHERE creator = '.$_SESSION['memberId:wse6'].' AND status = 3) as customerReply, (SELECT count(*) FROM ticket WHERE creator = '.$_SESSION['memberId:wse6'].' AND status = 0) as closedCount')->fetch();
?>
<style>
.form-material>label {
    position: absolute;
...
<div class="bg-primary">
    <div class="bg-pattern bg-black-op-25" style="background-image: url('assets/media/various/bg-pattern.png');">
        <div class="content text-center">
            <div class="pt-50 pb-20">
                <h1 class="font-w700 text-white mb-10">وسيط &mdash; الدعم الفني</h1>
                <h2 class="h4 font-w400 text-white-op">التذاكر</h2>
            </div>
        </div>
    </div>
</div>
    ...
    <h2 class="content-heading" style="padding-bottom: 10px;">
        ...
        <span id="ticketNameTwo">التذاكر (<?=$sql->rowCount()?>) </span>
    </h2>
    <div class="row">
        ...
        <div class="col-md-7 col-xl-9" id="TicketV">
            <div class="block">
                <div class="block-header block-header-default">
                    <div class="block-title text-right" id='ticketName'>
					التذاكر &mdash; [<?=$sql->rowCount()?>]
                    </div>
                ...
                <div class="block-content p-5" id="ticketInfo">
					<table class="table table-striped table-vcenter mb-0" id="ticketData">
							...
							<?php foreach($sql as $row){
								...
								if($row['type'] == 1){$type = 'إستفسار';}elseif($row['type'] == 2){$type = 'إقتراح';}elseif($row['type'] == 3){$type = 'شكوى على وسيط';}elseif($row['type'] == 4){$type = 'مشكلة';}elseif($row['type'] == 5){$type = 'طلب أرباح';}else{$type='none';}
                            ...
                            <div class="col-12">
                                <div class="form-material form-material-primary input-group">
                                    <select class="form-control" id="type">
                                        ...
                                        <option value="3">شكوى على وسيط</option>
...
var xmlhttp;
var ticketTypes = {1:"إستفسار", 2:"إقتراح", 3:"شكوى على وسيط", 4: "مشكلة"};
var ticketsDataTables;
	...
	var text = document.getElementById("text").value;	
		if(subject.length > 212 || text.length > 2000 || type.length > 1 || subject == '' || text == '' || type == ''){
			...
			if(type == 3){
				...
				swal({
				  title: 'إدخل إسم الوسيط في موقعنا',
				  type: 'question',
				  input: 'text',
				  inputPlaceholder: 'مثال: ML7oS',
				  showCancelButton: true,
				  confirmButtonText: 'إرسال',
				  cancelButtonText: 'إلغاء الأمر',
				  showLoaderOnConfirm: false,
				  inputValidator: (value) => {
					return !value && 'من فضلك ادخل الإسم'
				  },
				  inputAttributes: {
					maxlength: 32,
					autocapitalize: 'off',
				  ...
				  allowOutsideClick: () => !swal.isLoading()
					...
					if(result.value){
						sendData("Ticket.php", "subject="+subject+"&wse6="+result.value+"&type="+type+"&text="+text)
...
function readTicket(read, hey = true){
		...
		.then(function(response){
			if(response.show){
				...
				if(hey){
					$.notify({
					title: '<strong>تمت العملية!</strong>',
					message: 'الاطلاع على التذكرة رقم #'+response.id
				},{
					type: 'success'
				});
				}
...
function _backTickets(){
	...
	document.getElementById("ticketbuttons").innerHTML = '<button type="button" class="btn-block-option" data-toggle="block-option" data-action="fullscreen_toggle"><i class="si si-size-fullscreen"></i></button>';
